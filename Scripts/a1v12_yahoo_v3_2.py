...
def build_portfolios(comp, tv, static_models, tactical_models):
    """
    Compute raw-close price-return NAV and write the exact daily share ledger
    used by the NAV engine.

    The dividend engine reads this ledger directly. It does not recreate or
    estimate portfolio shares independently.
    """
    import pandas as pd
    import numpy as np
    ...
    for name, weights in all_models.items():
        ...
        # Initialize exact shares from the first raw-close price.
        shares = {}
        ...
        for i in range(len(df)):
            ...
            # Match the original NAV convention: rebalance on the first trading
            # day of a new year using the prior trading day's raw close.
    ...
    # Preserve the existing YTD anchor behavior in the displayed NAV file.
...
def build_dividend_analytics(comp_raw, comp_open, div_comp, sig, tv,
    ...
    """
    Build dividend reporting from an economic-account ledger.

    Price-return NAV remains based on raw closes and excludes distributions.
    Dividend income is recorded separately. For annually rebalanced models,
    accumulated dividend cash is included in the next annual rebalance so the
    invested base does not shrink merely because raw prices fall on ex-dates.

    Standalone buy-and-hold models retain their shares and accumulate cash.
    The tactical sleeve carries accumulated cash into the next tactical switch.
    """
    import pandas as pd
    import numpy as np
    ...
    # Build a synthetic tactical distribution-per-unit series.
    ...
    for i in range(len(prices)):
        ...
        if i > 0 and new_h != current:
            ...
            total_value = underlying_shares * old_open + tactical_cash
            underlying_shares = total_value / new_open
            tactical_cash = 0.0
            current = new_h
            period_start = i
            period_income = 0.0
    ...
    for model, weights in all_models.items():
        ...
        for asset, weight in keyed.items():
            ...
            if np.isfinite(px0) and px0 > 0:
                shares[asset] = BASE_VALUE * weight / px0

        cash_balance = 0.0
        current_year = int(prices.loc[0, "Date"].year)

        for i in range(len(prices)):
            dt = pd.to_datetime(prices.loc[i, "Date"])

            # Rebalance the full economic account at the prior close.
            if (i > 0 and model not in no_rebalance and len(shares) > 1
                ...
                account_value = security_value + cash_balance
                new_shares = {}
                for asset, weight in keyed.items():
                    px = float(prices.loc[prev_i, asset])
                    if np.isfinite(px) and px > 0:
                        new_shares[asset] = account_value * weight / px
                shares = new_shares
                cash_balance = 0.0
                current_year = int(dt.year)

            for asset, sh in shares.items():
                px = float(prices.loc[i, asset])
                share_rows.append({
                    "Date": dt,
                    "Model": model,
                    "Asset": asset,
                    "Shares": sh,
                    "Raw_Close": px,
                    "Position_Value": sh * px if np.isfinite(px) else 0.0,
    ...
    economic_ledger = pd.DataFrame(share_rows)
    events.to_csv(DATA / "Dividend_Daily_Income_Ledger.csv",
                  index=False, date_format="%Y-%m-%d")
    economic_ledger.to_csv(DATA / "Dividend_Economic_Share_Ledger.csv",
                           index=False, date_format="%Y-%m-%d")

    if events.empty:
        raise RuntimeError("Dividend event ledger is empty.")

    # Reporting layer: event ledger is the single source of truth.
    annual = (
        events.groupby(["Model", "Year"], as_index=False)["Dividend_Income"]
        .sum()
        .sort_values(["Model", "Year"])
    )
    annual["Cumulative_Income"] = (
        annual.groupby("Model")["Dividend_Income"].cumsum()
    )

    last_date = pd.to_datetime(prices["Date"].max())
    current_year = int(last_date.year)
    annual["Period"] = annual["Year"].astype(str)
    annual["Is_Partial_Year"] = "NO"
    mask_current = annual["Year"].eq(current_year)
    annual.loc[mask_current, "Period"] = (
        str(current_year) + " YTD through " + str(last_date.date())
    )
    annual.loc[mask_current, "Is_Partial_Year"] = "YES"
    annual.to_csv(DATA / "Dividend_Model_Annual.csv", index=False)

    monthly = (
        events.groupby(["Model", "Month"], as_index=False)["Dividend_Income"]
        .sum()
        .sort_values(["Model", "Month"])
    )
    monthly["Cumulative_Income"] = (
        monthly.groupby("Model")["Dividend_Income"].cumsum()
    )
    monthly.to_csv(DATA / "Dividend_Model_Monthly.csv", index=False)

    asset_annual = (
        events.groupby(["Model", "Year", "Asset"], as_index=False)["Dividend_Income"]
        .sum()
    )
    asset_monthly = (
        events.groupby(["Model", "Month", "Asset"], as_index=False)["Dividend_Income"]
        .sum()
    )
    ...
    for model, g in events.groupby("Model"):
        summary_rows.append({
            ...
            "Current_Year_Income": float(
                g.loc[g["Year"].eq(current_year), "Dividend_Income"].sum()),
            "Prior_Full_Year_Income": float(
                g.loc[g["Year"].eq(current_year - 1), "Dividend_Income"].sum()),
            "TTM_Dividend_Income": float(
                g.loc[g["Date"] > ttm_start, "Dividend_Income"].sum()),
            "Through_Date": last_date,
        })
    summary = pd.DataFrame(summary_rows)
    summary.to_csv(DATA / "Dividend_Model_Summary.csv",
                   index=False, date_format="%Y-%m-%d")

    # Reconciliation audit: event totals must match annual, monthly, and summary.
    audit_rows = []
    for model, g in events.groupby("Model"):
        event_total = float(g["Dividend_Income"].sum())
        annual_total = float(
            annual.loc[annual["Model"].eq(model), "Dividend_Income"].sum())
        monthly_total = float(
            monthly.loc[monthly["Model"].eq(model), "Dividend_Income"].sum())
        summary_total = float(
            summary.loc[summary["Model"].eq(model),
                        "Lifetime_Dividend_Income"].iloc[0])
        max_diff = max(
            abs(event_total - annual_total),
            abs(event_total - monthly_total),
            abs(event_total - summary_total),
        )
        audit_rows.append({
            "Model": model,
            "Event_Ledger_Total": event_total,
    ...
    pd.DataFrame(audit_rows).to_csv(
        AUDIT / "Dividend_Reconciliation_Audit.csv", index=False
    )

    # Coverage audit.
    coverage_rows = []
    for asset in sorted(set(events["Asset"])):
        a = events[events["Asset"].eq(asset)]
        coverage_rows.append({
            "Asset": asset,
            "Status": "PASS",
            "Distribution_Count": len(a),
            "First_Distribution_Date": a["Date"].min(),
            "Last_Distribution_Date": a["Date"].max(),
            "Total_Income_All_Models": float(a["Dividend_Income"].sum()),
        })
    pd.DataFrame(coverage_rows).to_csv(
        AUDIT / "Dividend_Coverage_Audit.csv",
        index=False, date_format="%Y-%m-%d"
    )

    # Fixed-income verification table.
    verify_rows = []
    for asset in ["PIMIX", "JPIE", "FIWDX", "JBND"]:
        ...
        if first is None:
            continue
        static_shares = BASE_VALUE / float(prices.loc[first, asset])
        tmp = pd.DataFrame({
            "Date": prices["Date"],
            "DPS": pd.to_numeric(divs[asset], errors="coerce").fillna(0.0),
        })
        tmp["Month"] = tmp["Date"].dt.to_period("M").astype(str)
        tmp["Income_on_100k_Initial_Investment"] = static_shares * tmp["DPS"]
        out = tmp.groupby("Month", as_index=False).agg(
            Dividend_Per_Share=("DPS", "sum"),
            Income_on_100k_Initial_Investment=(
                "Income_on_100k_Initial_Investment", "sum")
        )
        out.insert(0, "Asset", asset)
        verify_rows.append(out[out["Dividend_Per_Share"].abs() > 1e-12])

    verify = (
        pd.concat(verify_rows, ignore_index=True)
        if verify_rows else
        pd.DataFrame(columns=[
            "Asset", "Month", "Dividend_Per_Share",
            "Income_on_100k_Initial_Investment"
        ])
    )
...
<div class="card">
  <h2>Dividend Income <span class="pill">Income engine v8 economic-ledger verified</span></h2>
  <div class="note">Cash distributions are calculated from an economic-account share ledger. Accumulated cash is included at the next annual rebalance or tactical switch. Yield on cost is intentionally not displayed.</div>
...
<div class="card"><h2>Annual Income Detail</h2><div class="scroll"><table id="divAnnualTable"></table></div></div><div class="card"><h2>Dividend Reconciliation Audit</h2><div class="scroll"><table id="divReconTable"></table></div></div>
...
let tactical=parseCSV(EMBEDDED.tactical),portfolio=parseCSV(EMBEDDED.portfolio),signals=parseCSV(EMBEDDED.signals),trades=parseCSV(EMBEDDED.trades),holdsum=parseCSV(EMBEDDED.holdsum),holdperiods=parseCSV(EMBEDDED.holdperiods),audit=parseCSV(EMBEDDED.dataaudit),prodaudit=parseCSV(EMBEDDED.prodaudit),modelmap=parseCSV(EMBEDDED.modelmap),alloc=parseCSV(EMBEDDED.alloc),backfillaudit=parseCSV(EMBEDDED.backfillaudit),divsummary=parseCSV(EMBEDDED.divsummary),divannual=parseCSV(EMBEDDED.divannual),divasset=parseCSV(EMBEDDED.divasset),divperiods=parseCSV(EMBEDDED.divperiods),divrecon=parseCSV(EMBEDDED.divrecon);
...
function renderDividend(){
  ...
  document.getElementById('divKpis').innerHTML=
    ...
    `<div class=kpi><div class=label>Prior Full-Year Income</div><div class=big>${money(sum.Prior_Full_Year_Income)}</div><div class=note>Completed calendar year</div></div>`;
  ...
  drawTable('divReconTable',divrecon);
...
def main():
    print("BUILD MARKER: dividend-engine-v8-economic-ledger")
    ...
    build_dividend_analytics(comp_raw, comp_open, div_comp, sig, tv, portfolio_ledger, static_models, tactical_models)
