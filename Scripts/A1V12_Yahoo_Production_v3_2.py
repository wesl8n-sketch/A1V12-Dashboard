#!/usr/bin/env python3
"""
A1V13 Yahoo Production v4.1 (EMA89 Proximity Filter)

Complete integrated package.

v4.1 tactical signal enhancement:
- Raw-close MGK/MGV ratio vs EMA89.
- A crossover is accepted only when at least 1 of the prior 3 trading days
  was within ±0.30% of EMA89.
- Existing 3-day cooldown and T+1 open execution are preserved.
...
PROXIMITY_LOOKBACK_DAYS = 3
PROXIMITY_MIN_HITS = 1
...
def build_signals(comp_adj, comp_raw=None, signal_price_basis=SIGNAL_PRICE_BASIS):
    ...
    # Base regime: Growth when ratio is above EMA89, Value otherwise.
    raw_regime = (sig["MGK_MGV"] > sig["MGK_MGV_EMA89"]).astype(int)

    # v4.1 proximity filter:
    # Accept a crossover only when at least PROXIMITY_MIN_HITS of the prior
    # PROXIMITY_LOOKBACK_DAYS trading days were within ±PROXIMITY_THRESHOLD
    # of EMA89. The crossover day itself is deliberately excluded.
    sig["EMA89_Distance"] = (
        sig["MGK_MGV"] / sig["MGK_MGV_EMA89"] - 1.0
    ).abs()
    sig["Within_Proximity"] = (
        sig["EMA89_Distance"] <= PROXIMITY_THRESHOLD
    )
    sig["Prior_Proximity_Hits"] = (
        sig["Within_Proximity"]
        .shift(1)
        .rolling(PROXIMITY_LOOKBACK_DAYS, min_periods=PROXIMITY_LOOKBACK_DAYS)
        .sum()
        .fillna(0)
        .astype(int)
    )
    sig["Proximity_Qualified"] = (
        sig["Prior_Proximity_Hits"] >= PROXIMITY_MIN_HITS
    )

    n = len(sig)
    decision_position = [0] * n
    decision_position[0] = int(raw_regime.iloc[0])
    days_since = COOLDOWN_DAYS

    for i in range(1, n):
        days_since += 1
        crossover = raw_regime.iloc[i] != raw_regime.iloc[i - 1]
        qualified = bool(sig["Proximity_Qualified"].iloc[i])

        if (crossover and qualified
                and raw_regime.iloc[i] != decision_position[i - 1]
                and days_since >= COOLDOWN_DAYS):
            decision_position[i] = int(raw_regime.iloc[i])
            days_since = 0
        else:
    ...
    for i in range(1, n):
        ...
        if effective_position[i] != effective_position[i - 1]:
            ...
            rule = (f"Next trading day after qualified trigger (EMA89 crossover, "
                    f">={PROXIMITY_MIN_HITS} of prior {PROXIMITY_LOOKBACK_DAYS} "
                    f"days within ±{PROXIMITY_THRESHOLD:.2%}, "
    ...
    front = ["Date", "State", "EffectiveHolding", "Signal_Price_Basis",
             ...
             "EMA89_Distance", "Within_Proximity",
             "Prior_Proximity_Hits", "Proximity_Qualified"]
